...
class RuleTest extends PHPUnit_Framework_TestCase
{
	...
	public function testCanSetAndCheckRestrictionAgainstConditions()
	{
		...
		// Not specifically allowed to read object1
		$this->assertFalse($rule->isAllowed($object1));
		// Not specifically allowed to read object1
		$this->assertFalse($rule->isAllowed($object2));
		// Is specifically disallowed to read object1
		$this->assertTrue($rule->isDisallowed($object1));
		// Not specifically disallowed to read object1
		$this->assertFalse($rule->isDisallowed($object2));
		
		$rule->when(function($obj) { return 1 == 1; });

		$this->assertFalse($rule->isAllowed($object1));
		$this->assertFalse($rule->isAllowed($object2));
		// Is specifically disallowed to read object1, and 1==1
		$this->assertTrue($rule->isDisallowed($object1));
		$this->assertFalse($rule->isDisallowed($object2));
		...
		$this->assertFalse($rule->isAllowed($object1));
		$this->assertFalse($rule->isAllowed($object2));
		// Is specifically disallowed to read object1, and 1==1, but not 1==2
		$this->assertFalse($rule->isDisallowed($object1));
		$this->assertFalse($rule->isDisallowed($object2));
